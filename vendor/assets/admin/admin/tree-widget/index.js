...
(function($, context) {
  ...
  function DragWidget(root, options) {
    ...
    this.$contentWrapper  = this.$root.find('_content-wrapper');
    this.$content         = this.$root.find('_content');
    
    this.$content.find('> ul > li._node').data('drag-queen-root', true);
  ...
  DragWidget.nodeForElement = function(ele, ctor) {
    var node, $ele = $(ele).closest('._node');
  ...
  DragWidget.prototype = {
    ...
    expandAll: function() { this.$root.find('._actuator._parent:not(.expanded)').click(); },
    contractAll: function() { this.$root.find('._actuator._parent.expanded').click(); },
    
    reparent: function() {
      this.$content.find('._node').each(function() {
        ...
        if ($children.length == 1) {
          ...
          $('> ._item ._actuator', this)[hasChildren ? 'addClass' : 'removeClass']('_parent')
                              [childrenVisible ? 'addClass' : 'removeClass']('_expanded');
        }
      });
    },
    ...
    bind: function() {
      ...
      this.$content.delegate('._item', 'mouseover', function() {
        if (self.dragState == 'active') {
          if (self.nodeFor(this).acceptsDrop(self.getSelectedNodes())) {
            $(this).addClass('_target-valid');
          } else {
            $(this).addClass('_target-invalid');
          }
        }
      });
      
      this.$content.delegate('._item', 'mouseout', function() {
        $(this).removeClass('target-valid').removeClass('_target-invalid');
      });
      ...
      this.$content.delegate('._item ._actuator:not(._expanded)', 'mouseover', function() {
        if (self.dragState == 'active') {
          var ele = this;
      ...
      this.$content.delegate('._item ._actuator:not(._expanded)', 'mouseout', function() {
        springLoad = null;
      });
      ...
      this.$content.delegate('._item', 'mousedown', function(evt) {
        if ($(evt.target).is('._actuator')) return;
      ...
      this.$content.delegate('._item', 'mousemove', function(evt) {
      ...
      this.$content.delegate('._item', 'mouseup', function(evt) {
      ...
      this.$content.delegate('._reorder-target', 'mouseover', function() {
        if (self.dragState == 'active') {
      ...
      this.$content.delegate('._reorder-target', 'mouseout', function() {
        $(this).removeClass('_target-valid').removeClass('_target-invalid');
      });
      
      this.$content.delegate('._reorder-target', 'mouseup', function() {
      ...
      this.$content.click(function(evt) {
        ...
        if ($target.is('._parent._actuator')) {
    ...
    createDragElement: function(evt) {
      ...
      if (selection.length == 1 && this.options.useGhosting) {
        ...
        $ghost.find('._item').removeClass('_selected');
        
        this.$drag = $("<ul class='_drag-ghost'></ul>").html($ghost)
                                      .css({width: this.$content.width()});
      } else {
        this.$drag = $("<div class='_drag-badge' />").text(this.getSelection().length);
      }
    ...
    // ensure a node's children are loaded, firing callback on completion
      ...
      toggleVisibility: function(childList, show) {
          var expander = childList.prev('_item').find('._parent._actuator');
      if (!show && childList.is(':visible')) {
              childList.hide();
              expander.removeClass('_expanded');
          } else {
              childList.show();
              expander.addClass('_expanded');
          }
      },
      ...
      getSelection: function() {
          return this.$content.find('._item._selected');
      },

      clearSelection: function() {
      this.getSelection().removeClass('_selected');
      },

      makeSelected: function(ele) {
          $(ele).addClass('_selected');
      },

      toggleSelected: function(ele) {
          $(ele).toggleClass('_selected');
      },
